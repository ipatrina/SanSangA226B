...
Modem的配置数据或许也会使用闪存中的某个GPT分区存储，但是是由Modem固件程序处理的专有加密数据。

合约机出厂时，Modem被配置为仅限接入某个特定的(基站)运营商ID。

根据Modem镜像中的算法，会存在唯一解锁码。

向Modem固件程序提交正确的解锁码，Modem固件程序可以改写配置分区中的数据，取消运营商ID限制。

算法大概会涉及非对称加密。并且解锁码有尝试次数上限。已经尝试的次数同样会存储在配置分区。

另外，作为运营商向OEM定制的机型，以及Modem本身就是为运营商服务的产物，可能会有专用的(本运营商)基站数据通道(Modem直接接收)、或OEM厂商的Internet服务器(REE调用Modem接口)，来向Modem远程下发解锁指令。

因此，共有解锁码或远程下发两种方式。苹果手机仅提供后者的方式。
...
其实root提权和手机耗电没有什么本质关联。一些用户觉得root后的手机电池变得更不耐用，应该从以下几个方向排查。

- 类似Magisk、SuperSU方案的root权限管理器往往在实现时就非常囊肿，加之获得了REE最高权限，占山为王，脱离了应用后台使用限制，确实可能无端增加电池使用量。

- 但双5G也是现代安卓手机耗电增大的一个主要因素。将上网卡设置为5G，通话卡设置为4G，可有效改善手机耗电情况。

- 最后需要提及的就是安卓系统的通知推送机制了。

许多用户可能疑问，当安卓应用脱离OEM厂商提供的通知推送API(例如谷歌FCM)，尤其是在离线情况下，是否还可以随时向用户推送通知消息。

在标准的安卓实现中，应用想要向用户推送通知消息，有两种方案可供选择：应用自建后台服务运行于手机本地，或应用作者服务器调用OEM厂商通知推送API。

系统级应用属于前者实现，例如电话和短信APP。系统级应用不受电池使用量限制。

用户安装应用默认受电池使用量限制，但用户可以手动调整为无限制。在受限状态下，应用仍可在后台联网并推送通知消息，只不过会存在极大的推送延迟，原因是系统限制了应用后台服务的代码执行频率。用户手动调整为无限制后，则如同系统级应用一样，不会存在任何通知延迟，但一定会成为增加设备耗电的一个主要因素。

"通知推送极大延迟"或"增加手机耗电量"这二选一的代价，使应用开发者更倾向于采用后者实现通知推送。但使用前者、后者，或者两者的权利仍在开发者手中。

如果安卓(APK)应用的开发者编写了后台服务(Java)代码，则该应用进程有能力自手机启动(首次解锁)一直驻守，且系统UI中不出现任何痕迹，类似于Windows中的服务进程。但使用"ps -Af"命令可以查看进程。该进程有能力推送通知，且默认受电池使用量限制(除非用户手动更改为无限制)，可以在"应用程序信息"界面"强行停止"后台服务。

如果安卓(APK)应用的开发者编写了前台服务(Java)代码，则该应用进程有能力自手机启动(首次解锁)长期驻守在系统通知栏，类似于Windows中的托盘图标。该进程受更宽松的电池使用量限制(除非用户手动更改为无限制)。在点击多任务管理键后，"最近使用"界面不会出现该应用的活动窗口，但会出现在"N个活动应用程序"按钮所打开的"后台活动"列表，可以从中手动停止前台服务。

当然，系统级应用在手机启动后，无需解锁就可以直接驻守。OEM厂商的通知推送服务，就相当于单一系统级应用驻守了后台服务，使本应多个囊肿应用驻守后台变为单一精简应用。

若应用开发者未编写任何服务，则应用只能在窗口创建期间工作。

中国大陆用户经常使用的腾讯IM应用，仅采用了自建后台服务。而大多日常应用的开发者选择使用了OEM厂商API，例如谷歌FCM或华为Push Kit。

在中国大陆OEM生产的安卓智能手机(下称"国产手机")中，用户观察到的许多匪夷所思的现象往往基于不标准的安卓实现。

国产手机厂商为了所谓"让手机更耐用"，以及出于垄断原因，使用户即使将某应用设置为"无限制"，也可能遭受极为严苛的电池使用量限制，致其通知推送始终处于高延迟甚至不可用的状态。国产手机甚至完全屏蔽用户应用的前台和后台服务，致使应用无法正常工作。

这样的高压，迫使中国大陆应用开发者纷纷对接国产手机厂商的通知推送API，实现无延迟、高可靠通知推送，妥协于国产手机厂商的商业垄断。

而国产手机厂商却也要妥协于腾讯公司的垄断，因为其掌握着两款"国民级"IM应用。腾讯断定，国产手机厂商不敢放任QQ和微信的通知推送受到无理限制，厂商一定会自行解决这个问题，否则一旦设备无法正常接收日常IM消息，手机用户将不复存在。因此，QQ和微信并没有对接任何通知推送API，而是正常使用安卓后台服务的形式驻守。国产手机厂商只得将QQ和微信的应用包名加入系统级白名单，视为系统应用，不受电池使用量限制。这就产生了两个极端：两款特殊应用无需用户同意，直接进入"无限制"列表；其他应用则非"受限"，而是被彻底"拦截"，完全无法在后台工作。

据此，在三星手机等标准安卓实现中，若用户手动设定过多"无限制"应用，则将明显增加电池使用量。
...
在实践中，疑似其他RC服务使前者永远有"用户控制"权限被访问。只有存在"条件控制"的情况下，前者才能够无法被访问。而"条件控制"功能已经被我们砍废了。
